...
class Lattice {
   ...
   * @constructor
   */
  constructor( latticeConstants, orientation, options ) {
  ...
  /**
   * put functions here
   * @param {Object} [options] //NEW options passed in
   * @public
   */
  ...
  updateSites( options ) {
    ...
    //NEW option for rotatable node. Should default to false
    options = merge( {
      // @public - options provided to override default appearance of crystal
      rotatable: true  // give a square non-rotatable crystal. True cuts off corners
    }, options );
    ...
    //NEW @public read only
    this.crystalCorners = new Array( Math.ceil( crystalXMax - 1 ) );
    ...
    for ( let x = 0; x <= crystalXMax; x++ ) {
      for ( let y = 0; y <= crystalYMax; y++ ) {
        ...
        //NEW check that site is not too far from center for rotatable crystal.
        if ( options.rotatable && ( Math.sqrt(x*x*aLattice*aLattice+y*y*cLattice*cLattice) > CRYSTAL_SIZE ) ) {
          y = crystalYMax;
        }
        else {
          ...
          this.crystalCorners[x] = new Vector2( xCos - ySin, xSin + yCos );
        }
